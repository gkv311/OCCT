...
- **Visualization**
   ...
   - Redesigned `Image_AlienPixMap` - split into several implementations for different image libraries / formats
     (`Image_RWFreeImage` for FreeImage, `Image_RWAVCodec` for FFmpeg, `Image_RWWinCodec` for WinCodec)
     ...
     introduced `Image_RWAVCodec` for processing images via FFmpeg
     (extending previously available `Media_PlayerContext` for video playback
     and `Image_VideoRecorder` for recording video).
   ...
   - Fixed handling of multiple fonts within `Font_TextFormatter::Append()`;
     ...
     and improved documentation of vertical alignment options;
     fixed `AIS_TextLabel` selection area in several cases (multi-line text and vertical alignment).
   - Improved WebGL compatibility and added test group `webgl2` for `EGL_ANGLE_create_context_webgl_compatibility`.
- **Configuration**
