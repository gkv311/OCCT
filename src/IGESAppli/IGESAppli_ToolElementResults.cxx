  ...
  // UNFINISHED
{
  ...
  for (Standard_Integer i = 1; i <= ne; i ++) {
    ...
    if (dlf < 4 && ent->NbLayers(i) != 1) {
      Snprintf(mess,"Nb. of Layers n0.%d not ONE while Data Layer Flag is in [0-3]",i);
      ach->AddFail(mess);
    }
    if (rrf == 1 || rrf == 2)
      if (nrl != 1 || ent->ResultDataLoc(i,1) != 0) {
	Snprintf(mess,"Result Data Locs n0.%d incorrect for Result Report = 1 or 2",i);
	ach->AddFail(mess);
      }
    if (ent->NbResults(i) != (nv*nl*nrl)) {
      Snprintf(mess,"Nb. of results for Element n0.%d incorrect, should be %d",
	      i,nv*nl*nrl);
      ach->AddFail(mess);
    }
  }
