...
{
#if defined(HAVE_WAYLAND)
  return myWlDisplay.get() != nullptr ? wl_display_get_fd(myWlDisplay.get()) : -1;
#else
  return -1;
#endif
}
                                     ...
                                     const TCollection_AsciiString& theDisplayName)
{
  ...
  {
    // setup global error callback
    wl_log_set_handler_client([](const char* theFormat, va_list theArgList) -> void {
      char aBuffer[1024] = {};
      vsnprintf(aBuffer, sizeof(aBuffer), theFormat, theArgList);
      Message::SendFail() << "Wayland error: " << aBuffer;
    });
  }
    ...
    .global = [](void* theData,
      ...
      if (::strcmp(theInterface, "wl_output") == 0)
      {
        ...
        static const wl_output_listener anOutListener =
        {
          ...
          .done = [](void* , wl_output* ) {},
          .scale = [](void* theScaleData, wl_output* theOutput, int32_t theFactor)
          {
            Wayland_DisplayConnection* aScaleThis = (Wayland_DisplayConnection*)theScaleData;
            for (WlOutputInfo& aScaleOutput : aScaleThis->myWlOutputs)
            {
              if (aScaleOutput.Output == theOutput)
              {
                aScaleOutput.Scale = theFactor;
                break;
              }
            }
          },
      ...
      if (::strcmp(theInterface, "wl_seat") == 0)
      {
        ...
        {
          .capabilities = [](void* theSeatData, wl_seat* theSeat, uint32_t theCaps)
          {
            Wayland_DisplayConnection* aSeatThis = (Wayland_DisplayConnection*)theSeatData;
            if ((theCaps & WL_SEAT_CAPABILITY_POINTER) != 0 && !aSeatThis->myWlPointer)
              aSeatThis->myWlPointer.reset(wl_seat_get_pointer(theSeat),
                                           [](wl_pointer* theObj) { theObj ? wl_pointer_destroy(theObj) : (void)theObj; });
            else if ((theCaps & WL_SEAT_CAPABILITY_POINTER) == 0 && aSeatThis->myWlPointer)
              aSeatThis->myWlPointer.reset();

            if ((theCaps & WL_SEAT_CAPABILITY_KEYBOARD) != 0 && !aSeatThis->myWlKeyboard)
              aSeatThis->myWlKeyboard.reset(wl_seat_get_keyboard(theSeat),
                                            [](wl_keyboard* theKey) { theKey ? wl_keyboard_destroy(theKey) : (void)theKey; });
            else if ((theCaps & WL_SEAT_CAPABILITY_KEYBOARD) == 0 && aSeatThis->myWlKeyboard)
              aSeatThis->myWlKeyboard.reset();
          },
      ...
      if (::strcmp(theInterface, xdg_wm_base_interface.name) == 0)
      {
        ...
        {
          .ping = [](void* thePingData, xdg_wm_base* theXdgWmBase, uint32_t theSerial)
          {
            (void )thePingData;
...
int Wayland_DisplayConnection::Flush()
{
  ...
  int aNbFlushed = -1;
#if defined(HAVE_WAYLAND)
  aNbFlushed = wl_display_dispatch_pending(myWlDisplay.get());
  wl_display_flush(myWlDisplay.get());
#endif
  return aNbFlushed;
}
