...
namespace {
  ...
  static int getPerformantCoresMask(DWORD_PTR& theMask)
  {
    ...
    // so that we might want to keep only cores above the minimum EfficiencyClass.
    // However, AMD Ryzen that doesn't have any 'slow' cores varies EfficiencyClass per core,
    // making this logic irrelevant (without involving other mechanism).
    const bool toUseSched = false; //!toUseEff && (aMaxSched > aMinSched);
